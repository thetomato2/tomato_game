...
recanonicalize_coord(i32 &tile_, f32 &tile_rel_)
{
    ...
    i32 offset { round_f32_to_i32(tile_rel_ / (f32)Tile_Map::s_tile_size_meters) };
...
static Tile_Chunk *
get_tile_chunk(Tile_Map &tile_map_, const i32 tile_chunk_x_, const i32 tile_chunk_y_,
               const i32 tile_chunk_z_, Mem_Arena *arena_ = nullptr)
{
    ...
    i32 hash_val  = 19 * tile_chunk_x_ + 7 * tile_chunk_y_ + 3 * tile_chunk_z_;
    i32 hash_slot = hash_val & (ArrayCount(tile_map_.tile_chunk_hash) - 1);
    ...
    do {
        ...
        if (arena_ && chunk->x == TILE_CHUNK_UNITIALIZED && !chunk->next_in_hash) {
            chunk->next_in_hash = PushStruct(arena_, Tile_Chunk);
            chunk               = chunk->next_in_hash;
            chunk->x            = TILE_CHUNK_UNITIALIZED;
        }

        // if chunk is empty (0) allocate the tiles
        if (arena_ && chunk->x == TILE_CHUNK_UNITIALIZED) {
            chunk->tiles = PushArray(arena_, Tile_Map::s_chunk_tile_count_total, i32);
...
static i32
get_tile_value_unchecked(const Tile_Chunk &tile_chunk_, const i32 tile_x_, const i32 tile_y_)
{
...
static i32
get_tile_value(Tile_Chunk *const tile_chunk_, const i32 tile_x_, const i32 tile_y_)
{
    i32 tile_value {};
...
static i32
get_tile_value(Tile_Map &tile_map_, const i32 abs_tile_x_, const i32 abs_tile_y_,
               const i32 abs_tile_z_)
{
    i32 tile_value {};
...
{
    i32 tile_value { get_tile_value(tile_map_, pos_.abs_tile_x, pos_.abs_tile_y, pos_.abs_tile_z) };
    return tile_value;
}

static void
set_tile_value_unchecked(Tile_Chunk &tile_chunk_, const i32 tile_x_, const i32 tile_y_,
                         const i32 tile_value_)
{
    TOM_ASSERT(tile_chunk_.tiles);
    TOM_ASSERT(tile_x_ <= Tile_Map::s_chunk_tile_count && tile_y_ <= Tile_Map::s_chunk_tile_count);
...
static void
set_tile_value(Tile_Chunk *tile_chunk_, const i32 abs_tile_x_, const i32 abs_tile_y_,
               const i32 tile_value_)
{
    if (tile_chunk_ && tile_chunk_->tiles) {
...
static void
set_tile_value(Mem_Arena *arena_, Tile_Map &tile_map_, const i32 abs_tile_x_, const i32 abs_tile_y_,
               const i32 abs_tile_z_, const i32 tile_value_)
{
    Tile_Chunk_Pos chunk_pos { get_chunk_pos(abs_tile_x_, abs_tile_y_, abs_tile_z_) };
    ...
    if (!tile_chunk->tiles) {
        tile_chunk->tiles = PushArray(arena_, Tile_Map::s_chunk_tile_count_total, i32);
        for (i32 tile_ind {}; tile_ind < Tile_Map::s_chunk_tile_count_total; ++tile_ind) {
...
static bool
is_tile_value_empty(const i32 tile_value_)
{
...
static Tile_Map_Pos
get_centered_tile_point(const i32 abs_tile_x_, const i32 abs_tile_y_, const i32 abs_tile_z_)
{
    Tile_Map_Pos result {};
...
{
    for (i32 tile_chunk_i = 0; tile_chunk_i < ArrayCount(tile_map_.tile_chunk_hash);
         ++tile_chunk_i) {
        tile_map_.tile_chunk_hash[tile_chunk_i].x = TILE_CHUNK_UNITIALIZED;  // null chunk
    }
};
