...
internal u32 DS5_crc32_compute(byt *buffer, szt len)
{
...
internal void DS5_enum_devices(void *ptr_buf, u32 arr_len, u32 *ds5_cnt)
{
    ...
    if (!hiddi_hnd || (hiddi_hnd == INVALID_HANDLE_VALUE)) {
        PRINT_WARN("Could not find HID devices");
        return;
    }
...
u32 DS5_init(DS5_Context *out)
{
    ...
    DS5_enum_devices(info, ARR_CNT(info), &controller_cnt);

    if (controller_cnt) {
        PRINT_INFO(str_fmt("Found %u DualSense controllers.", controller_cnt));
...
internal void DS5_process_button(Button *but, byt is_down)
{
...
internal void DS5_process_buttons(DS5_State *state, byt buttons_and_dpad, byt buttons_a, byt buttons_b)
{
...
internal void DS5_parse_hid_buffer(byt *hid_buf, DS5_State *state)
{
...
internal void DS5_process_trigger(DS5_TriggerEffect *effect, byt *buffer)
{
    // Switch on effect
    switch (effect->type) {
        // Continuous
...
internal void DS5_create_hid_output_buffer(byt *hid_buf, DS5_State *state)
{
    hid_buf[0x00] = 0xFF;
    hid_buf[0x01] = 0xF7;
...
void ds5_get_input(DS5_Context *context, DS5_State *state)
{
    if (!context->connected) {
        PRINT_INFO(str_fmt("DS5 controller %u was disconnected.", context->port));
        return;
    }
    ...
    if (context->connection == DS5_Connection::BT) {
        ...
        if (SUCCEEDED(ReadFile(context->hnd, context->hid_buf, report_len, NULL, NULL)))
            ...
            PRINT_ERR(str_fmt("Failed to read Hid buffer for DS5 controller %u!", context->port));
#endif
    } else {
        if (SUCCEEDED(ReadFile(context->hnd, context->hid_buf, report_len, NULL, NULL))) {
...
void ds5_push_output(DS5_Context *context, DS5_State *state)
{
    ...
    if (FAILED(WriteFile(context->hnd, context->hid_buf, report_len, NULL, NULL))) {
        PRINT_ERR(str_fmt("Failed to write Hid buffer for DS5 controller # %u!", context->port));
    }
}
